...
export default function DashboardHome() {
  ...
  const filteredTeamStats = rawTeamStats.filter((t) => {
    const nameMatch = (t.teamName || "").toLowerCase().includes(teamSearch.toLowerCase());
  ...
  const teamPieData = filteredTeamStats.map((t) => ({
  ...
  const kizhaiGraphData = kizhais
    .map(k => ({
      ...
      percentage: Math.round(((k.memberCount || 0) / (k.targetCount || 100)) * 100)
    }))
    .sort((a, b) => b.percentage - a.percentage) 
    .slice(0, 7); 

  // Overall Target Calculation
  const overallTarget = kizhais.reduce((acc, k) => acc + (k.targetCount || 0), 0);
  const overallAchieved = kizhais.reduce((acc, k) => acc + (k.memberCount || 0), 0);
  const overallPercentage = overallTarget ? Math.round((overallAchieved / overallTarget) * 100) : 0;
      ...
      <DashboardLayout>
        <div className="space-y-6 animate-pulse">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[1, 2, 3].map((i) => (<div key={i} className="h-32 bg-gray-900/60 rounded-2xl border border-gray-800" />))}
          </div>
  ...
  return (
    <DashboardLayout>
      <div className={`min-h-screen text-gray-100 transform transition-all duration-700 ease-out ${mounted ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}>
        
        {/* ================= LEADERSHIP BANNER (4 CARDS) ================= */}
        <div className="grid grid-cols-1 md:grid-cols-12 gap-4 mb-10 items-stretch">
          {/* 1. GENERAL SECRETARY */}
          <div className="md:col-span-3 bg-gradient-to-br from-gray-950 to-black border border-gray-800 rounded-2xl p-4 flex flex-col items-center justify-center text-center shadow-[0_12px_35px_rgba(0,0,0,0.6)] relative overflow-hidden group transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_18px_50px_rgba(0,0,0,0.9)] hover:border-amber-400/60">
            ...
            <div className="w-20 h-20 rounded-full border-2 border-gray-600 overflow-hidden shadow-md mb-3">
              <img src={GensecImg} alt="Gen Sec" className="w-full h-full object-cover object-top grayscale group-hover:grayscale-0 transition-all duration-500 scale-[1.2]" />
            </div>
            <div>
          ...
          {/* 2. THALAIVAR */}
          <div className="md:col-span-6 bg-gradient-to-b from-[#3b0202] via-[#4a0404] to-[#1a0000] border-2 border-amber-400/70 rounded-2xl p-6 flex flex-col items-center justify-center text-center shadow-[0_0_55px_rgba(245,158,11,0.45)] relative overflow-hidden z-10 min-h-[220px]">
            ...
            <div className="relative z-10">
              ...
              <h2 className="text-2xl md:text-3xl font-extrabold text-white font-tamil drop-shadow-[0_4px_12px_rgba(0,0,0,0.7)]">தளபதி திரு. விஜய்</h2>
              <div className="inline-flex items-center gap-2 bg-gradient-to-r from-amber-400 to-orange-500 text-black text-[10px] font-black px-4 py-0.5 rounded-full mt-2 uppercase tracking-wider shadow-[0_0_20px_rgba(251,191,36,0.8)]">
                <span className="w-1.5 h-1.5 rounded-full bg-red-600 animate-pulse" /> President / தலைவர்
              </div>
            </div>
          </div>
          ...
          <div className="md:col-span-3 flex flex-col gap-4">
            ...
            <div className="flex-1 bg-gradient-to-bl from-gray-950 to-black border border-amber-500/40 rounded-2xl p-3 flex items-center gap-3 shadow-[0_10px_30px_rgba(0,0,0,0.7)] relative overflow-hidden group transition-all duration-300 hover:-translate-y-1 hover:border-amber-300">
              <div className="absolute top-0 right-0 bg-gradient-to-r from-amber-400 to-orange-500 text-black text-[8px] font-black uppercase tracking-wider px-2 py-0.5 rounded-bl-lg shadow-sm z-10">District Leader</div>
        ...
        <div className="mb-8 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            <h1 className="text-3xl font-bold bg-gradient-to-r from-white via-amber-200 to-orange-400 bg-clip-text text-transparent">Overview</h1>
            <p className="text-sm text-gray-500 mt-1">Real-time analytics and team performance metrics.</p>
          </div>
          ...
          <div className="flex flex-col md:flex-row md:items-center gap-3 w-full md:w-auto">
            ...
            <div className="flex-1 md:flex-none flex items-center gap-2 bg-gray-900/80 border border-gray-700 rounded-full px-3 py-1.5 shadow-sm">
              ...
              <input value={teamSearch} onChange={(e) => setTeamSearch(e.target.value)} placeholder="Search teams..." className="bg-transparent outline-none text-xs text-gray-200 placeholder:text-gray-500 w-full" />
            </div>

            {/* Filters */}
            <div className="flex items-center gap-1 bg-gray-900/70 border border-gray-800 rounded-full px-1 py-1 text-[11px]">
              {[{ id: "all", label: "All" }, { id: "high", label: "High Strength" }, { id: "low", label: "Low Strength" }].map((f) => (
                <button key={f.id} onClick={() => setTeamFilter(f.id)} className={`px-2.5 py-0.5 rounded-full transition-colors ${teamFilter === f.id ? "bg-gradient-to-r from-amber-500 to-orange-500 text-black font-semibold" : "text-gray-400 hover:text-gray-200"}`}>
                  {f.label}
                </button>
              ))}
            </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="relative">
                <button onClick={() => setProfileOpen((prev) => !prev)} className="flex items-center gap-2 bg-gray-900/80 border border-gray-700 rounded-full px-2.5 py-1.5 hover:border-amber-400/70 transition-all">
                  <div className="w-7 h-7 rounded-full bg-gradient-to-br from-amber-400 to-orange-500 flex items-center justify-center text-xs font-semibold text-black">S</div>
                  <div className="hidden sm:block text-left">
                    <p className="text-[11px] font-medium text-gray-100">Admin</p>
                    <p className="text-[10px] text-gray-500">District Coordinator</p>
                  </div>
                ...
                {profileOpen && (
                  <div className="absolute right-0 mt-2 w-44 rounded-xl bg-gray-950 border border-gray-800 shadow-2xl py-2 text-xs text-gray-200 z-20">
                    <button className="w-full text-left px-3 py-2 hover:bg-gray-800/70 flex items-center gap-2"><UserCircle2 size={14} /> View Profile</button>
                    <button className="w-full text-left px-3 py-2 hover:bg-gray-800/70 flex items-center gap-2"><Activity size={14} /> Activity Log</button>
                    <div className="border-t border-gray-800 my-1" />
                    <button className="w-full text-left px-3 py-2 hover:bg-red-600/20 text-red-400 flex items-center gap-2"><span className="h-2 w-2 rounded-full bg-red-500 animate-pulse" /> Logout</button>
                  </div>
                )}
              </div>
            </div>
        ...
        {error && (
          <div className="mb-6 p-4 bg-red-500/10 border border-red-500/20 rounded-xl text-red-400 flex items-center gap-3">
            <span className="h-2 w-2 rounded-full bg-red-500 animate-pulse" /> {error}
          </div>
        )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
          ...
          <ChartCard title="Member Distribution" subtitle="By Team (filtered)">
            <div className="flex flex-col md:flex-row items-center h-full">
              <div className="w-full h-64 md:w-1/2">
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    <Pie data={teamPieData} dataKey="value" nameKey="name" cx="50%" cy="50%" innerRadius={60} outerRadius={80} paddingAngle={5} cornerRadius={4}>
              ...
              <div className="w-full md:w-1/2 md:pl-4 space-y-3 h-64 overflow-y-auto custom-scrollbar">
                {filteredTeamStats.length === 0 && <p className="text-xs text-gray-500">No teams match your search / filter.</p>}
                {filteredTeamStats.map((team, idx) => (
                  <div key={team.teamName} className="flex items-center justify-between text-sm group">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-gray-300 font-medium group-hover:text-white transition-colors truncate max-w-[150px]">{team.teamName}</span>
                    </div>
            ...
            {!maanadu || maanaduTeamPieData.length === 0 ? (
              ...
              <div className="flex flex-col md:flex-row items-center h-full">
                <div className="w-full h-64 md:w-1/2">
                  <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
                      <Pie data={maanaduTeamPieData} dataKey="value" nameKey="name" cx="50%" cy="50%" innerRadius={60} outerRadius={80} paddingAngle={5} cornerRadius={4}>
                ...
                <div className="w-full md:w-1/2 md:pl-4 space-y-3 h-64 overflow-y-auto custom-scrollbar">
                  {maanaduTeamPieData.sort((a, b) => b.value - a.value).map((t, idx) => (
                    <div key={t.name} className="flex items-center justify-between text-sm group">
                      <div className="flex items-center gap-2">
                        <span className="w-3 h-3 rounded-full shadow-sm" style={{ backgroundColor: COLORS[idx % COLORS.length] }}></span>
                        <span className="text-gray-300 font-medium group-hover:text-white transition-colors truncate max-w-[150px]">{t.name}</span>
                      </div>
        ...
        {/* ✅ NEW SECTION: KIZHAI PERFORMANCE GRAPH */}
        <div className="grid grid-cols-1 gap-6">
           <ChartCard 
             title="Kizhai Performance (Top 7)" 
             subtitle={`Overall Target Achieved: ${overallAchieved} / ${overallTarget} (${overallPercentage}%)`}
             action={<div className="bg-red-900/20 p-1.5 rounded-lg border border-red-500/30"><Target size={16} className="text-red-400" /></div>}
           >
              <div className="h-72 w-full mt-4">
                 <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={kizhaiGraphData} margin={{ top: 10, right: 30, left: 0, bottom: 0 }}>
                      ...
                      />
                      <Legend />
                      <Bar dataKey="members" name="Achieved" fill="#10b981" radius={[4, 4, 0, 0]} barSize={20} />
                      <Bar dataKey="target" name="Target Goal" fill="#374151" radius={[4, 4, 0, 0]} barSize={20} />
                    </BarChart>
                 </ResponsiveContainer>
              </div>
           </ChartCard>
  ...
  return (
    <div className="bg-gray-900/50 backdrop-blur-xl border border-gray-800 rounded-2xl p-6 shadow-xl transition-all duration-300 hover:-translate-y-1.5 hover:shadow-[0_18px_45px_rgba(0,0,0,0.9)] hover:border-amber-400/70">
      <div className="flex items-start justify-between mb-4">
        <div className="p-3 rounded-xl border border-amber-400/30 bg-amber-500/5">{icon}</div>
